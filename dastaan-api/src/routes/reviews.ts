...
export function createReviewInvite(bookingId: string): string | null {
  const b = db.prepare(
    ...
    | undefined;
  if (!b) return null;
  const existing = db.prepare("SELECT token FROM reviews WHERE booking_id = ?").get(bookingId) as
...
export function barberRating(barberId: string): { average: number | null; count: number } {
  const r = db.prepare(
  ...
  return { average: r.avg ? Math.round(r.avg * 10) / 10 : null, count: r.n };
}
...
export default async function reviewRoutes(app: FastifyInstance) {
  ...
  app.post("/reviews", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (req, reply) => {
    ...
    const r = db.prepare("SELECT id, submitted_at FROM reviews WHERE token = ?").get(token) as
    ...
    db.prepare("UPDATE reviews SET rating = ?, comment = ?, submitted_at = ? WHERE id = ?")
  ...
  app.get("/reviews", async (req, reply) => {
    ...
    return s.role === "admin"
      ? db.prepare(`${base} AND r.branch_id = ? ORDER BY r.submitted_at DESC LIMIT 100`).all(s.branchId)
      : db.prepare(`${base} ORDER BY r.submitted_at DESC LIMIT 100`).all();
  });
}
