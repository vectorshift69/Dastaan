...
export default async function clientRoutes(app: FastifyInstance) {
  ...
  app.get("/clients", async (req, reply) => {
    ...
    const sql = `
      SELECT b.client_id AS id,
             ...
             (b.client_id IS NOT NULL) AS registered
      FROM bookings b
      WHERE (? IS NULL OR b.branch_id = ?)
        AND (? = '%%' OR lower(b.client_name) LIKE ? OR lower(COALESCE(b.client_phone,'')) LIKE ?)
      GROUP BY COALESCE(b.client_id, b.client_name)
      ORDER BY lastVisit DESC
      LIMIT 200`;
    const rows = db.prepare(sql).all(branchId, branchId, term, term, term) as ClientRow[];

    return rows.map((r) => ({
      ...r,
      registered: !!r.registered,
      loyalty: r.id ? loyaltyForClient(r.id) : null,
    }));
  });
  ...
  app.get("/clients/:id", async (req, reply) => {
    ...
    const history = (s.role === "admin"
      ? db.prepare(historySql).all(id, s.branchId)
      : db.prepare(historySql).all(id)) as Record<string, unknown>[];

    return {
      ...user,
      loyalty: loyaltyForClient(id),
      history: history.map((h) => ({
        ...h,
        paid: !!h.paid,
        services: (JSON.parse(h.serviceIds as string) as string[]).map(
          (sid) => (db.prepare("SELECT name FROM services WHERE id = ?").get(sid) as { name: string } | undefined)?.name ?? "Service"
        ),
      })),
    };
  });
  ...
  app.patch("/clients/:id", async (req, reply) => {
    ...
    const existing = db.prepare("SELECT name, phone FROM users WHERE id = ? AND role = 'client'").get(id) as
    ...
    audit("client_updated", { actorId: s.sub, actorRole: s.role, detail: id, ip: req.ip });
    return { ok: true };
  });
  ...
  app.post("/clients", async (req, reply) => {
    ...
    db.prepare("INSERT INTO users (id, role, name, phone, created_at) VALUES (?,?,?,?,?)")
      .run(id, "client", parsed.data.name, parsed.data.phone ?? null, now());
    audit("client_created", { actorId: s.sub, actorRole: s.role, detail: parsed.data.name, ip: req.ip });
