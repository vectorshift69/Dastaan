...
export default async function reportRoutes(app: FastifyInstance) {
  ...
  app.get("/reports/barber/me", async (req, reply) => {
    const s = requireRole(req, reply, ["barber"]);
    if (!s) return;
    ...
    for (const b of bookings) {
      const inv = db.prepare("SELECT gross FROM invoices WHERE booking_id = ?").get(b.id) as { gross: number } | undefined;
    ...
    return {
      ...
      rating: barberRating(s.sub), // { average, count } — PRD 7 "ratings"
    };
  });
  ...
  app.get("/reports/timeline", async (req, reply) => {
    ...
    const rows = branchId
      ? db.prepare("SELECT date, branch_id AS branchId, data, created_at AS archivedAt FROM day_snapshots WHERE date = ? AND branch_id = ?").all(q.date, branchId)
      : db.prepare("SELECT date, branch_id AS branchId, data, created_at AS archivedAt FROM day_snapshots WHERE date = ?").all(q.date);
  ...
  app.post("/reports/snapshot", async (req, reply) => {
    ...
    const count = snapshotDay(date);
    audit("timeline_snapshot", { actorId: s.sub, actorRole: s.role, detail: date, ip: req.ip });
    return { date, branchesArchived: count };
  });
}

/* Archive the full booking state of a day, per branch (upsert = re-running is safe) */
export function snapshotDay(date: string): number {
...
export function snapshotIfDayRolled() {
  const nowDate = today();
  if (nowDate !== lastSeenDate) {
    snapshotDay(lastSeenDate);
